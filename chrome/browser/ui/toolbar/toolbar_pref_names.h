...
// Indicates whether the Monti side panel action has been auto-pinned to the
// toolbar for existing profiles (the default-pin only applies to new profiles).
inline constexpr char kPinnedMontiMigrationComplete[] =
    "toolbar.pinned_monti_migration_complete";

// Indicates whether the (now redundant) Monti toolbar button has been
// auto-unpinned. The Monti control now lives as a labeled status chip in the
// location bar, so the separate pinned button is removed once per profile.
inline constexpr char kUnpinnedMontiMigrationComplete[] =
    "toolbar.unpinned_monti_migration_complete";
