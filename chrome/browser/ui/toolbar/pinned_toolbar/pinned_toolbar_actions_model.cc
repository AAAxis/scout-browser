...
void PinnedToolbarActionsModel::MaybeMigrateExistingPinnedStates() {
  ...
  // The Monti control now lives as a labeled status chip in the location bar,
  // so the separate pinned toolbar button is redundant. Auto-unpin it once per
  // profile (including those where the earlier auto-pin migration ran).
  if (!pref_service_->GetBoolean(prefs::kUnpinnedMontiMigrationComplete)) {
    UpdatePinnedState(kActionSidePanelShowMonti, false);
    pref_service_->SetBoolean(prefs::kUnpinnedMontiMigrationComplete, true);
    pref_service_->SetBoolean(prefs::kPinnedMontiMigrationComplete, true);
  }
}
