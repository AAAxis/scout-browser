...
std::u16string BrowserView::GetAccessibleWindowTitleForChannelAndProfile(
  ...
  // Monti-launched windows (see MontiManager::OnBrowserCreated) set an
  // explicit user title to the profile's own name, since there's no longer a
  // separate per-profile app identity to tell windows apart by. That name is
  // already the complete, self-sufficient identity for this window -- return
  // it as-is instead of falling through to the tab-title/browser-name/
  // incognito-annotation formatting below, which would needlessly append the
  // generic "Monti" product name to it.
  const std::string& user_title =
      WindowMetadataController::From(browser_.get())->user_title();
  if (!user_title.empty()) {
    return base::UTF8ToUTF16(user_title);
  }
