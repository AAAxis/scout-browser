...
bool IsMontiProfileLaunch(const base::CommandLine& command_line) {
  return command_line.HasSwitch("monti-profile-launch") &&
         !command_line.HasSwitch(switches::kAppId);
}

// External launchers (e.g. the Monti Anty Electron app) spawn the browser
// directly with a raw `--proxy-server=socks5://host:port` (or http://) switch
// for the assigned proxy. Chromium's native SOCKS5 client does not implement
// RFC 1929 username/password auth, and a bare `--proxy-server` never seeds the
// HttpAuthCache either, so an authenticated proxy passed this way always fails
// (ERR_SOCKS_CONNECTION_FAILED, or a 407 for http). The launcher additionally
// passes the raw endpoint and credentials via `--monti-proxy-*` switches
// specifically so the browser can route the connection through
// MontiProfileService::Connect() instead: that path starts the local
// SocksBridge (which performs the authenticated handshake upstream while
// presenting an unauthenticated SOCKS5 listener to Chromium) and applies the
// resulting local port as the profile's proxy, exactly as the in-app
// chrome://monti manager already did before it was removed.
//
// Reachability/auth verification of the assigned proxy is Monti Anty's job --
// it checks before ever spawning this process -- so the browser no longer
// gates navigation on Connect()'s result; it just fires the bridge/auth setup
// and returns. Returns nullopt when --monti-proxy-host is absent, i.e. no
// proxy was assigned at all.
std::optional<monti::MontiProxy> MontiProxyFromCommandLine(
    const base::CommandLine& command_line) {
  if (!command_line.HasSwitch("monti-proxy-host")) {
    return std::nullopt;
  }
  monti::MontiProxy proxy;
  proxy.host = command_line.GetSwitchValueASCII("monti-proxy-host");
  proxy.transport = command_line.HasSwitch("monti-proxy-type")
                        ? command_line.GetSwitchValueASCII("monti-proxy-type")
                        : "http";
  int port = 0;
  base::StringToInt(command_line.GetSwitchValueASCII("monti-proxy-port"),
                    &port);
  if (proxy.transport == "socks5") {
    proxy.socks_port = port;
  } else {
    proxy.http_port = port;
  }
  proxy.username = command_line.GetSwitchValueASCII("monti-proxy-user");
  proxy.password = command_line.GetSwitchValueASCII("monti-proxy-pass");
  proxy.id = command_line.HasSwitch("monti-profile-id")
                 ? command_line.GetSwitchValueASCII("monti-profile-id")
                 : proxy.host;
  return proxy;
}

// Local page MontiLocalApiServer serves (see its OnHttpRequest) when the
// binary is started without going through Monti Anty at all. Never the real
// target/login/direct page.
GURL MontiUnauthorizedLaunchUrl() {
  return GURL("http://127.0.0.1:39217/v1.0/internal/unauthorized-launch");
}
...
void StartupBrowserCreatorImpl::DetermineURLsAndLaunch(
  ...
  // Start the local REST API server once per process so that external
  // automation scripts can manage profiles, proxies, and cookies via HTTP.
  if (process_startup == chrome::startup::IsProcessStartup::kYes) {
    monti::MontiLocalApiServer::GetInstance()->Start();
  }

  // Monti is only ever meant to run as a profile Monti Anty spawned (with a
  // proxy and fingerprint already resolved into command-line switches). A
  // fresh process (process_startup) that reaches here without
  // --monti-profile-launch -- e.g. someone double-clicked Monti.app directly,
  // ran the binary from a shell, or an attacker tried to get an unprotected
  // window out of it -- would otherwise silently open a normal browsing
  // session with the host's real IP and fingerprint. Refuse instead. Scoped
  // to process_startup so it never fires for new windows/tabs opened inside
  // an already-running, already-authorized session, and excludes --app-id/
  // --app so legitimate app-mode/PWA launches (which never carry Monti
  // switches anyway) aren't affected.
  if (process_startup == chrome::startup::IsProcessStartup::kYes &&
      !IsMontiProfileLaunch(*command_line_) &&
      !command_line_->HasSwitch(switches::kAppId) &&
      !command_line_->HasSwitch(switches::kApp)) {
    LOG(ERROR) << "Monti started without --monti-profile-launch; refusing to "
                 "open a normal browsing session.";
    tabs = StartupTabs({StartupTab(MontiUnauthorizedLaunchUrl())});
    RestoreOrCreateBrowser(tabs, BrowserOpenBehavior::NEW,
                           /*restore_options=*/0, process_startup,
                           is_post_crash_launch);
    return;
  }

  // Non-nullopt only for an Monti launch with an assigned proxy: applied via
  // Connect() fire-and-forget below, once the real browser/tab exist -- see
  // the comment above MontiProxyFromCommandLine for why this no longer gates
  // navigation on the result.
  std::optional<monti::MontiProxy> pending_monti_proxy;
  if (IsMontiProfileLaunch(*command_line_)) {
    if (result.launch_result != LaunchResult::kWithGivenUrls) {
      tabs = StartupTabs({StartupTab(GURL("chrome://newtab"))});
    }

    // Apply the fingerprint before any window/tab is created, so it is
    // already in prefs by the time MontiUaTabHelper and the renderer read it
    // back.
    if (std::optional<monti::Fingerprint> fp =
            monti::DecodeRuntimeFingerprintSwitch(*command_line_)) {
      monti::MontiProfileServiceFactory::GetForProfile(profile_)
          ->SetFingerprint(monti::ToDict(*fp));
    }

    pending_monti_proxy = MontiProxyFromCommandLine(*command_line_);
    // If absent and not free-proxy mode, there's simply no proxy to apply --
    // Monti Anty already verified the assignment before spawning this
    // process, so nothing further to do here; navigate normally.
  }
  ...
  if (pending_monti_proxy) {
    // Starts the SocksBridge and applies the resulting authenticated local
    // proxy to the profile. Deliberately ApplyAssignedProxy(), not Connect():
    // the latter also verifies egress and reverts to a direct connection on
    // failure, which would silently drop this profile onto an unproxied
    // connection with nothing surfaced to the user now that this no longer
    // gates/redirects the tabs RestoreOrCreateBrowser already navigated above
    // -- see the comment above MontiProxyFromCommandLine.
    monti::MontiProfileServiceFactory::GetForProfile(profile_)
        ->ApplyAssignedProxy(*pending_monti_proxy);
  }
