...
void AddChromeColorMixers(ui::ColorProvider* provider,
  ...
  // Monti warm-neutral toolbar/bookmark overrides. After Material so it wins
  // over the default surface colors, before native so user themes still win.
  AddMontiColorMixer(provider, key);
