...
base::DictValue CreateProfileState(const ProfileAttributesEntry* entry,
  ...
  // Monti enrichment: join this Chrome profile to its Monti identity by the
  // profile directory basename (MontiProfileEntry.id == Chrome profile dir).
  // All lookups below are cheap in-memory reads (UI-thread safe).
  bool is_monti = false;
  const std::string profile_dir = entry->GetPath().BaseName().AsUTF8Unsafe();
  monti::MontiManager* monti_manager = monti::MontiManager::GetInstance();
  for (const monti::MontiProfileEntry& ap : monti_manager->GetProfiles()) {
    if (ap.id != profile_dir) {
      continue;
    }
    is_monti = true;
    // UA preset key: "windows" | "macos" | "linux" | "" (no override).
    profile_entry.Set("fingerprintPreset", ap.ua_preset);
    std::string proxy_country;
    if (!ap.assigned_proxy_id.empty()) {
      const monti::MontiProxy* proxy =
          monti::ProxyStore::GetInstance()->GetById(ap.assigned_proxy_id);
      if (proxy) {
        proxy_country = proxy->country;
      }
    }
    profile_entry.Set("proxyCountry", proxy_country);
    profile_entry.Set("running", monti_manager->IsRunning(ap.id));
    break;
  }
  profile_entry.Set("isMonti", is_monti);
