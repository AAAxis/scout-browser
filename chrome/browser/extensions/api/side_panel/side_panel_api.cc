...
ExtensionFunction::ResponseAction SidePanelOpenFunction::RunFunction() {
  ...
  // `sidePanel.open()` requires a user gesture -- except for an Monti Free
  // Proxy launch (--monti-free-proxy), where the bundled FoxyWall Proxy
  // extension auto-opens its own panel on startup so the connection attempt
  // is visible instead of silent. That switch is only ever present on a
  // profile the user explicitly launched in Free Proxy mode, so this isn't a
  // general gesture bypass for arbitrary extensions/web content.
  if (!user_gesture() &&
