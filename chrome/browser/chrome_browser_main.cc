...
void ChromeBrowserMainParts::PostProfileInit(Profile* profile,
  ...
  // Monti: eagerly start the manager so its global browser observer is live
  // before any window opens. This guarantees the per-profile UA + fingerprint
  // identity is applied however the profile window is created (picker, session
  // restore after restart, external-link handler), not only when the profile
  // picker happens to have instantiated the manager first.
  if (is_initial_profile) {
    monti::MontiManager::GetInstance();
  }
