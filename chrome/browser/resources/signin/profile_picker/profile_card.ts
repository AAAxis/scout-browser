...
export class ProfileCardElement extends ProfileCardElementBase {
  ...
  // Human-readable label for the Argus fingerprint UA preset
  // ("windows" | "macos" | "linux" | ""). Empty hides the preset chip.
  protected presetLabel_(): string {
    switch (this.profileState.fingerprintPreset) {
      case 'windows':
        return 'Windows';
      case 'macos':
        return 'macOS';
