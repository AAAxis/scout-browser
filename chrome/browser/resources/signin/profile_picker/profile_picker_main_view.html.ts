...
<div class="flex-container">
  ...
  <div id="profilesWrapper" ?hidden="${(this.shouldHideProfilesWrapper_())}">
    <div id="profilesContainer" class="custom-scrollbar">
      ${this.getProfileRows_().map((row) => 'header' in row ? html`
        <div class="profile-group-label">${row.header}</div>
      ` : html`
        <profile-card class="profile-item" data-index="${row.index}"
            .profileState="${row.item}" .disabled="${this.pickerButtonsDisabled_}"
