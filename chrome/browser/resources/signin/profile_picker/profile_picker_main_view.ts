...
export class ProfilePickerMainViewElement extends
  ...
  // Builds the ordered list of rows rendered in the profiles container,
  // splitting Argus identities from plain Google/Chrome profiles (session 22).
  // Each card row keeps its original index into `profilesList_` so drag/menu/
  // launch behaviour is unchanged. Group headers are emitted only when both
  // groups are present, so a default-only profile set renders the flat stock
  // list with no header.
  protected getProfileRows_():
      Array<{header: string}|{item: ProfileState, index: number}> {
    const cards = this.profilesList_.map((item, index) => ({item, index}));
    const argus = cards.filter(card => card.item.isArgus);
    const google = cards.filter(card => !card.item.isArgus);
    const showHeaders = argus.length > 0 && google.length > 0;
    const rows: Array<{header: string}|{item: ProfileState, index: number}> =
        [];
    if (showHeaders) {
      rows.push({header: loadTimeData.getString('argusIdentitiesGroupLabel')});
    }
    rows.push(...argus);
    if (showHeaders) {
      rows.push({header: loadTimeData.getString('googleProfilesGroupLabel')});
    }
    rows.push(...google);
    return rows;
  }
