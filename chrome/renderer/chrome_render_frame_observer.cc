...
void ChromeRenderFrameObserver::DidCreateNewDocument() {
  // Monti: inject the per-profile fingerprint spoof into the main world at
  // document-start (before page scripts), for every frame that carries one.
  if (blink::WebView* web_view = render_frame()->GetWebView()) {
    const std::string& fingerprint =
        web_view->GetRendererPreferences().monti_fingerprint_json;
    if (!fingerprint.empty()) {
      monti::InjectFingerprintSpoof(render_frame()->GetWebFrame(), fingerprint);
    }
  }
