...
struct BLINK_COMMON_EXPORT RendererPreferences {
  ...
  // Monti: per-profile fingerprint as a compact JSON string. Empty when the
  // profile has no fingerprint override. The renderer parses it at
  // document-start to spoof JS-layer surfaces (canvas/audio/WebGL/screen/etc.).
  std::string monti_fingerprint_json;
