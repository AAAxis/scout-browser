...
# IMPORTANT: For update instructions, see README.md.
{
  ...
  "<(SHARED_INTERMEDIATE_DIR)/chrome/browser/resources/side_panel/monti/resources.grd": {
    "META": {"sizes": {"includes": [20],}},
    "includes": [4765],
  },
 ...
 "<(SHARED_INTERMEDIATE_DIR)/ash/webui/projector_app/resources/app/untrusted/resources.grd": {
  ...
  # Monti full-page Home (chrome://monti). Slotted into the unused 10141-11999
  # window. Allocated a wide range (400) because the Proxies tab bundles the
  # full country-flag SVG set (~257 files) as static resources. resource_ids.spec
  # requires ids to ascend in file order, so the later monti blocks below start
  # after this 11000-11399 range.
  "<(SHARED_INTERMEDIATE_DIR)/chrome/browser/resources/monti/resources.grd": {
    "META": {"sizes": {"includes": [400]}},
    "includes": [11000],
  },

  # Monti new-tab page (chrome://monti-newtab). Slotted into the same unused
  # 10141-11999 window, just after the Monti Home block.
  "<(SHARED_INTERMEDIATE_DIR)/chrome/browser/resources/monti_newtab/resources.grd": {
    "META": {"sizes": {"includes": [30]}},
    "includes": [11400],
  },

  # Monti account settings popup (chrome://monti-account). Slotted into the same
  # unused 10141-11999 window, just after the Monti new-tab block.
  "<(SHARED_INTERMEDIATE_DIR)/chrome/browser/resources/monti_account/resources.grd": {
    "META": {"sizes": {"includes": [30]}},
    "includes": [11500],
  },

  # Monti settings window (chrome://monti-settings). Slotted into the same
  # unused 10141-11999 window, just after the Monti account block.
  "<(SHARED_INTERMEDIATE_DIR)/chrome/browser/resources/monti_settings/resources.grd": {
    "META": {"sizes": {"includes": [30]}},
    "includes": [11600],
  },
